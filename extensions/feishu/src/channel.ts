...
const FILE_APPROVAL_DEFAULT_TTL_MS = 10 * 60 * 1000;
type FileApproval = {
  id: string;
  action: string;
  path: string;
...
const pendingFileApprovals = new Map<string, FileApproval>();
const approvedFileApprovals = new Map<string, number>();
...
function cleanupFileApprovals(now: number): void {
  for (const [id, approval] of pendingFileApprovals) {
    if (approval.expiresAt <= now) {
      pendingFileApprovals.delete(id);
    }
  }
  for (const [id, expiresAt] of approvedFileApprovals) {
    if (expiresAt <= now) {
      approvedFileApprovals.delete(id);
    }
  }
}
...
function parseApprovalCommand(text: string): { action: "approve" | "deny"; id: string } | null {
  ...
  const match = /^(?:\/)?(approve|deny)\s+([a-zA-Z0-9_-]+)$/i.exec(trimmed);
...
function shouldLogContent(cfg: FeishuChannelConfig | undefined): boolean {
  return cfg?.logMessageContent === true;
}

function isAdmin(cfg: FeishuChannelConfig | undefined, senderId: string): boolean {
  return !!cfg?.adminIds?.includes(senderId);
}

function checkFilePathAllowed(
  cfg: FeishuChannelConfig | undefined,
  filePath: string
): { ok: boolean; error?: string } {
  const access = cfg?.fileAccess;
  const resolved = normalizePath(filePath);
  const whiteList = (access?.whiteList || []).map((p) => normalizePath(p));
  const blackList = (access?.blackList || []).map((p) => normalizePath(p));

  for (const deny of blackList) {
    if (isPathWithin(resolved, deny)) {
      return { ok: false, error: `Path is blocked by blackList: ${filePath}` };
    }
  }

  if (whiteList.length > 0) {
    const allowed = whiteList.some((allow) => isPathWithin(resolved, allow));
    if (!allowed) {
      return { ok: false, error: `Path is not in whiteList: ${filePath}` };
    }
  }
...
async function guardLocalFileAccess(opts: {
  cfg?: ClawdbotConfig;
  ...
  const feishuCfg = opts.cfg ? getFeishuConfig(opts.cfg) : undefined;
  const pathCheck = checkFilePathAllowed(feishuCfg, opts.filePath);
  ...
  const now = Date.now();
  cleanupFileApprovals(now);

  if (opts.approvalId) {
    const expiresAt = approvedFileApprovals.get(opts.approvalId);
    if (expiresAt && expiresAt > now) {
      approvedFileApprovals.delete(opts.approvalId);
      return { ok: true };
    }
    return { ok: false, error: `Approval ID invalid or expired: ${opts.approvalId}` };
  }

  const approvalId = buildApprovalId();
  const ttl = feishuCfg?.fileAccess?.approvalTimeoutMs ?? FILE_APPROVAL_DEFAULT_TTL_MS;
  pendingFileApprovals.set(approvalId, {
    id: approvalId,
    action: opts.action,
    path: normalizePath(opts.filePath),
    createdAt: now,
    expiresAt: now + ttl,
  });

  const approvalChatId = feishuCfg?.fileAccess?.approvalChatId;
  if (approvalChatId) {
    const notice = [
      "⚠️ 文件访问需要管理员审批",
      `action: ${opts.action}`,
      ...
      "管理员可回复: /approve <approvalId> 或 /deny <approvalId>",
    ].join("\n");
    await sendTextMessage(opts.account, approvalChatId, notice);
  }

  return {
    ok: false,
    error: `File access requires approval. approvalId=${approvalId}`,
  };
}
...
const feishuMessageActions: ChannelMessageActionAdapter = {
  ...
  handleAction: async (ctx) => {
    ...
    const cfg = ctx.cfg as ClawdbotConfig | undefined;
    const { action } = ctx;
    ...
    switch (action) {
      ...
      case "send_image": {
        const guard = await guardLocalFileAccess({
          cfg,
          account,
          action,
          filePath: ctx.filePath as string,
          approvalId: ctx.approvalId as string | undefined,
        });
      ...
      case "file_download": {
        const guard = await guardLocalFileAccess({
          ...
          account,
          action,
          filePath: ctx.savePath as string,
          approvalId: ctx.approvalId as string | undefined,
        });
        if (!guard.ok) return { ok: false, error: guard.error };
      ...
      // AI 能力
      case "ai_ocr": {
        const image = ctx.image as string;
        ...
        return recognizeImage(account, image);
      }

      case "ai_speech_to_text": {
        const audio = ctx.audio as string;
        ...
        return speechToText(account, audio, {
...
export const feishuPlugin: ChannelPlugin<ResolvedFeishuAccount> = {
  ...
  meta: {
    ...
    docsPath: "https://github.com/Ricky-Yu-QQQ/openclaw",
  ...
  outbound: {
    ...
    sendMedia: async (ctx) => {
      ...
      if (mediaUrl && !mediaUrl.startsWith("http")) {
        ...
        if (!guard.ok) {
          return { ok: false, error: new Error(guard.error) };
        }
  ...
  gateway: {
    startAccount: async (ctx) => {
      ...
      const cfg = ctx.cfg;
      const feishuCfg = getFeishuConfig(cfg);
      const logContent = shouldLogContent(feishuCfg);
      ...
      startGateway({
        ...
        onMessage: async (message) => {
          ...
          const approvalCmd = rawText ? parseApprovalCommand(rawText) : null;
          if (approvalCmd) {
            if (!isAdmin(feishuCfg, message.senderId)) {
              await sendTextMessage(account, message.chatId, "❌ 你没有审批权限");
              return;
            }

            const now = Date.now();
            cleanupFileApprovals(now);
            const pending = pendingFileApprovals.get(approvalCmd.id);
            if (!pending) {
              await sendTextMessage(
                account,
                message.chatId,
                `⚠️ 未找到待审批记录: ${approvalCmd.id}`
              );
              return;
            }
          ...
          if (message.chatType !== "p2p") {
            const hasMention = (message.mentions?.length || 0) > 0;
            if (!hasMention) {
              if (logContent) {
                logger.info("忽略群聊消息：未 @");
              } else {
                logger.info("忽略群聊消息");
              }
              return;
            }
          } else if (feishuCfg?.allowFrom && feishuCfg.allowFrom.length > 0) {
            if (!feishuCfg.allowFrom.includes(message.senderId)) {
              logger.info("忽略私聊消息：不在 allowlist");
              return;
            }
          }
          ...
          switch (message.messageType) {
            case "text":
              ...
              logger.info(logContent ? `收到文本消息: ${bodyText}` : "收到文本消息");
              break;

            case "image":
              if (message.imageKey) {
                logger.info(logContent ? `收到图片消息: ${message.imageKey}` : "收到图片消息");
                try {
                  ...
                  if (imgResult.ok && imgResult.data) {
                    ...
                    bodyText = "[图片]";
                    logger.info(logContent ? `图片已下载: ${filePath}` : "图片已下载");
                  } else {
            ...
            case "file":
              if (message.fileKey) {
                ...
                logger.info(logContent ? `收到文件消息: ${fileName}` : "收到文件消息");
                try {
                  ...
                  if (fileResult.ok && fileResult.data) {
                    ...
                    logger.info(logContent ? `文件已下载: ${filePath}` : "文件已下载");
                  } else {
            ...
            case "audio":
              if (message.audioRecognition) {
                ...
                logger.info(
                  logContent
                    ? `收到语音消息 (${durationSec}s): ${bodyText.slice(0, 50)}...`
                    : `收到语音消息 (${durationSec}s)`
                );
              } else {
                // 没有转写结果，提示用户
                const durationSec = message.audioDuration
            ...
            case "post":
              ...
              try {
                ...
                logger.info(
                  logContent ? `收到富文本消息: ${bodyText.slice(0, 100)}...` : "收到富文本消息"
                );
              } catch {
                bodyText = "[富文本消息]";
              }
