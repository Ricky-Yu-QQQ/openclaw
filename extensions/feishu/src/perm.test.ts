  ...
  createFeishuClient: vi.fn(() => ({
    drive: {
      permissionMember: {
        ...
        create: (...args: any[]) => addImpl(...args),
        delete: (...args: any[]) => removeImpl(...args),
      },
    },
  })),
}));
...
describe("feishu perm tools", () => {
  ...
  it("skips when perm disabled", () => {
    const api = buildApi({
      channels: { feishu: { appId: "app", appSecret: "secret", tools: { perm: false } } },
    });
    registerFeishuPermTools(api);
    expect(Object.keys(api.tools)).toHaveLength(0);
  });
  ...
  it("registers and executes perm actions", async () => {
    const api = buildApi({
      channels: { feishu: { appId: "app", appSecret: "secret", tools: { perm: true } } },
    });
    registerFeishuPermTools(api);
    ...
    listImpl.mockResolvedValue({
      code: 0,
      data: { items: [{ member_type: "openid", member_id: "ou_1", perm: "view", name: "A" }] },
    });
    ...
    const addRes = await tool.execute("id", {
      action: "add",
      token: "t",
      type: "doc",
      member_type: "openid",
    ...
    const removeRes = await tool.execute("id", {
      action: "remove",
      token: "t",
      type: "doc",
      member_type: "openid",
