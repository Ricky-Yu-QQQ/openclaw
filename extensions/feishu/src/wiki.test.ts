...
vi.mock("./client.js", () => ({
  createFeishuClient: vi.fn(() => ({
    wiki: {
      space: { list: (...args: any[]) => listSpacesImpl(...args), getNode: (...args: any[]) => getNodeImpl(...args) },
      spaceNode: {
...
describe("feishu wiki tools", () => {
  ...
  it("skips when wiki disabled", () => {
    const api = buildApi({ channels: { feishu: { appId: "app", appSecret: "secret", tools: { wiki: false } } } });
    registerFeishuWikiTools(api);
    expect(Object.keys(api.tools)).toHaveLength(0);
  });

  it("registers and executes wiki actions", async () => {
    const api = buildApi({ channels: { feishu: { appId: "app", appSecret: "secret", tools: { wiki: true } } } });
    registerFeishuWikiTools(api);
    const tool = api.tools.feishu_wiki;

    listSpacesImpl.mockResolvedValue({ code: 0, data: { items: [{ space_id: "s1", name: "Space" }] } });
    const spaces = await tool.execute("id", { action: "spaces" });
    expect(spaces.details.spaces[0].space_id).toBe("s1");

    listNodesImpl.mockResolvedValue({ code: 0, data: { items: [{ node_token: "n1", obj_token: "o1", obj_type: "docx", title: "T" }] } });
    const nodes = await tool.execute("id", { action: "nodes", space_id: "s1" });
    expect(nodes.details.nodes[0].node_token).toBe("n1");

    getNodeImpl.mockResolvedValue({ code: 0, data: { node: { node_token: "n1", space_id: "s1" } } });
    const get = await tool.execute("id", { action: "get", token: "n1" });
    expect(get.details.space_id).toBe("s1");

    createNodeImpl.mockResolvedValue({ code: 0, data: { node: { node_token: "n2", obj_token: "o2", obj_type: "docx", title: "New" } } });
    ...
    const renamed = await tool.execute("id", { action: "rename", space_id: "s1", node_token: "n2", title: "Renamed" });
