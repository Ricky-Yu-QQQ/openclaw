...
- Direct chats follow `session.dmScope` (default `per-peer`); guild channels stay isolated as `agent:<agentId>:discord:channel:<channelId>` (display names use `discord:<guildSlug>#<channelSlug>`).
